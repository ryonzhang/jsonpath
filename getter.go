...
func JsonPathLookup(obj interface{}, jpath string) (interface{},[]string, error) {
	c, err := Compile(jpath)
	if err != nil {
		return nil,nil, err
	}
...
func (c *Compiled) Lookup(obj interface{}) (interface{},[]string, error) {
	var err error
	var gJsons []string
	for _, s := range c.steps {
		...
		case "idx":
			...
			if len(s.args.([]int)) > 1 {
				...
				for _, x := range s.args.([]int) {
					...
					res = append(res, tmp)
					tmpGJson=append(tmpGJson,appendKey(gJsons,strconv.Itoa(x))...)
				}
				obj = res
				gJsons = tmpGJson
				...
				gJsons =appendKey(gJsons,strconv.Itoa(s.args.([]int)[0]))
				if err != nil {
					return nil,nil, err
				}
			} else {
				//fmt.Println("idx ----------------4")
				return nil,nil, fmt.Errorf("cannot index on empty slice")
			}
		case "range":
			if len(s.key) > 0 {
				// no key `$[:1].test`
			...
			if argsv, ok := s.args.([2]interface{}); ok == true {
				...
				return nil,nil, fmt.Errorf("range args length should be 2")
			}
		case "filter":
			obj, err = get_key(obj, s.key)
			if err != nil {
			...
			gJsons =appendKey(gJsons,s.key)
			var ids []int
			obj,ids, err = get_filtered(obj, obj, s.args.(string))
			var tmpGJson []string
			for ind := range ids {
				tmpGJson = append(tmpGJson, appendKey(gJsons, strconv.Itoa(ind))...)
			}
			gJsons = tmpGJson
			if err != nil {
				return nil,nil, err
		...
		default:
			return nil,nil, fmt.Errorf("expression don't support in filter")
		}
	}
	return obj, gJsons ,nil
}

func appendKey(array []string,key string) (result []string){
	for _,s:=range array{
...
func get_filtered(obj, root interface{}, filter string) ([]interface{},[]int, error) {
	var nums []int
	...
	case reflect.Map:
		if op == "=~" {
			...
			for i, kv := range reflect.ValueOf(obj).MapKeys() {
	...
	default:
		return nil,nil, fmt.Errorf("don't support filter on this type: %v", reflect.TypeOf(obj).Kind())
	}
